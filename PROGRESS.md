...
- [x] M3.4 M3 验收（延迟基准入档 docs/PERFORMANCE.md；M3.3 control-mode 列为可选后续）
- [ ] M4.1 Visual
- [ ] M4.2 搜索
- [ ] M4.3 寄存器
- [ ] M4.4 . 重复
...
- 2026-09-06 10:1x M2.5 验收达成：隔离 socket 实测 -enter→d(NAV [delete])→
  ...
  列入 M3.1）。
- 2026-09-06 10:5x M3.1：post-key 同步移入串行队列（修复时序竞争）+
  删 loadState 往返 + reconcile 条件化。每键 exec 11-13 → ≤5。
- 2026-09-07 03:3x M3.2+M3.4：基准入档 docs/PERFORMANCE.md——
  daemon 内存态 1.5-4μs/键；端到端 23.7ms/键（进程 fork 主体，
  对比优化前 40-60ms）；M3.3 control-mode 列可选后续。
